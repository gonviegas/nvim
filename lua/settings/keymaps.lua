...
map(
  ...
  opts,
  "BUFFERS",
  "buffer_manager.toggle_quick_menuBdelete",
  "Open buffer list"
)
map("n", "<A-S-c>", "<CMD>Bdelete<CR>", opts, "BUFFERS", "Bdelete", "Close current buffer")
map("n", "<A-q>", "<CMD>BufferLineCyclePrev<CR>", opts, "BUFFERS", "BufferLineCyclePrev", "Previous Buffer Focus")
map("n", "<A-w>", "<CMD>BufferLineCycleNext<CR>", opts, "BUFFERS", "BufferLineCycleNext", "Next Buffer Focus")
map(
  "n",
  "<A-S-w>",
...
end, opts, "LSP", "diagnostic.goto_prev_error", "Go to the previous error diagnostic")
map("n", "g}", function()
  require("lspsaga.diagnostic"):goto_next({ severity = vim.diagnostic.severity.ERROR })
end, opts, "LSP", "diagnostic.goto_next_error", "Go to the next error diagnostic")
map(
  "n",
  "<space>e",
...
map(
  ...
  opts,
  "LSP-TROUBLE",
  "TroubleToggle_lsp_implementations",
...
map(
  ...
  opts,
  "LSP-TROUBLE",
  "TroubleToggle_lsp_type_definitions",
...
map(
  ...
  opts,
  "LSP-TROUBLE",
  "TroubleToggle_document_diagnostics",
...
map(
  ...
  opts,
  "LSP-TROUBLE",
  "TroubleToggle_workspace_diagnostics",
  "Show workspace diagnostics"
)
map("n", "<space>f", lsp_formatting, opts, "LSP", "lsp.buf.formatting", "[F]ormat code")
map(
  "n",
  "<leader>ds",
...
-- REST
map("n", "<leader>rr", "<Plug>RestNvim<CR>", opts, "REST", "RestNvim", "Run the request under the cursor")
map("n", "<leader>rl", "<Plug>RestNvimLast<CR>", opts, "REST", "RestNvimLast", "Re-run the last request")
map("n", "<leader>rp", "<Plug>RestNvimPreview<CR>", opts, "REST", "RestNvimPreview", "Preview the request cURL command")
...
map(
  ...
  "TELESCOPE",
  "Telescope_current_buffer_fuzzy_find",
...
map(
  ...
  opts,
  "REACT-EXTRACT",
  "React_Extract_New_File",
  "React Extract to new file"
)
map(
  { "v" },
  "<Leader>rc",
  ...
  opts,
  "REACT-EXTRACT",
  "React_Extract_Current_File",
  "React Extract to current file"
)
...
function _G.set_terminal_keymaps()
  ...
  vim.keymap.set("t", "<A-C-h>", [[<C-\><C-n> | 4<C-w><<CR>]], nobufopts)
  vim.keymap.set("t", "<A-C-l>", [[<C-\><C-n> | 4<C-w>><CR>]], nobufopts)
  vim.keymap.set("t", "<A-C-j>", [[<C-\><C-n> | 2<C-w>+<CR>]], nobufopts)
  vim.keymap.set("t", "<A-C-k>", [[<C-\><C-n> | 2<C-w>-<CR>]], nobufopts)
  vim.keymap.set("t", "<A-S-h>", [[<C-\><C-n> | <Cmd>WinShift left<CR>]], nobufopts)
  vim.keymap.set("t", "<A-S-l>", [[<C-\><C-n> | <Cmd>WinShift right<CR>]], nobufopts)
  vim.keymap.set("t", "<A-S-j>", [[<C-\><C-n> | <Cmd>WinShift down<CR>]], nobufopts)
  vim.keymap.set("t", "<A-S-k>", [[<C-\><C-n> | <Cmd>WinShift up<CR>]], nobufopts)
end
